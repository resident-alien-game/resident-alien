using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class CopControl : MonoBehaviour
{
    public float moveSpeed = 6f;
    public float changeDirectionInterval = 2f;

    private float timer;
    private Vector3 randomDirection;
    public float groundSize = 40f; // Size of the ground
    private GameObject alien;
    private FormChange formChange;
    private StatusManagement status;
    private CivilianControl civilian;

    void Start()
    {
        timer = changeDirectionInterval;
        GetNewRandomDirection();
        status = GameObject.FindObjectOfType<StatusManagement>();
        civilian = GameObject.FindObjectOfType<CivilianControl>();
    }

    void Update()
    {
        formChange = GameObject.FindObjectOfType<FormChange>();
        timer -= Time.deltaTime;

        if (timer <= 0f && !formChange.isAlienForm())
        {
            GetNewRandomDirection();
            timer = changeDirectionInterval;
        }
        Move();
    }

    void Move()
    {
        if (formChange.isAlienForm())
        {
            ChaseAlien();
        }
        else
        {
            Vector3 movement = randomDirection.normalized * moveSpeed * Time.deltaTime;
            transform.Translate(movement);

            // Clamp object's position to stay within the ground area
            Vector3 clampedPosition = transform.position;
            clampedPosition.x = Mathf.Clamp(clampedPosition.x, -groundSize / 2f, groundSize / 2f);
            clampedPosition.z = Mathf.Clamp(clampedPosition.z, -groundSize / 2f, groundSize / 2f);
            transform.position = clampedPosition;
        }
    }

    void GetNewRandomDirection()
    {
        randomDirection = new Vector3(Random.Range(-1f, 1f), 0f, Random.Range(-1f, 1f)).normalized;
    }

    void ChaseAlien()
    {
        alien = GameObject.FindObjectOfType<FormChange>().gameObject;

        if (civilian.isDead())
        {
            moveSpeed = moveSpeed * 2;
        }
        Vector3 direction = alien.transform.position - transform.position;
        transform.Translate(direction.normalized * moveSpeed * Time.deltaTime);
    }

    void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.tag == "Alien")
        {
            status.ReduceHP(1);
        }
    }
}
